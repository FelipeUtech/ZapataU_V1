...
def generar_malla_refinada(dominio, zapata, params):
    ...
    # Suelo existe desde z=0 (superficie), habrá hueco rectangular en área zapata
...
def generar_malla_gradual(dominio, zapata, params):
    ...
    # Capas de suelo: desde z=0 (superficie) hacia abajo
    # El suelo EXISTE desde z=0, pero habrá un HUECO rectangular en área de zapata
...
def crear_elementos_con_zapata(nx, ny, nz, nodes_per_layer, x_coords, y_coords, z_coords,
    ...
    # Calcular límites de estratos desde z=0 (superficie) hacia abajo
    # El suelo existe desde la superficie; la excavación es solo un hueco rectangular
    if estratos_suelo:
        limites_estratos = []
        z_acum = 0.0  # Empezar desde superficie del terreno
    ...
    for k in range(nz):
        for j in range(ny):
            for i in range(nx):
                ...
                # Verificar si está en área horizontal de zapata
